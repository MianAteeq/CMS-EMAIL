...
<div class="row">
    <div class="col-md-3">
        ...
        <p style="color: black;padding-left: 25px;">Status :  @if ($user['application_status']=="IN_REVIEW") Info in Review @else {{$user['application_status']}}  @endif</p>
          <p style="color: black;padding-left: 25px;">Verification status :  @if($user['profile']['business_status']==0)  Pending @elseif ($user['profile']['business_status']==1) Accepted  @else Decline @endif</p>

          @if($user['profile']['business_status']==0)
          <div class="footers" style="border-top: 2px solid black;padding: 10px ">
            <button class="btn btn-primary btn-block" onclick="window.location.href=`{{route('admin.business.verify',$user['id'])}}`">ACCEPT</button>
            <button class="btn btn-primary btn-block" onclick="window.location.href=`{{route('admin.business.decline',$user['id'])}}`">DECLINE</button>
                ...
                <div class="card-content">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-7 text-secondary">
                                {{$user['profile']['address_line_1']}} , {{$user['profile']['address_line_2']}} , {{$user['profile']['city']}}  , {{$user['profile']['postcode']}}
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-sm-5">
                                <h6 class="mb-0">Sole Trader / Self Employed Proof</h6>
                            </div>
                            <div class="col-sm-7 text-secondary">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-7 text-secondary">
                             @if($user['profile']['business_status']==0)  Pending @elseif ($user['profile']['business_status']==1) Accepted  @else Decline @endif
                            </div>
                        </div>
                        @if($user['profile']['business_status']==0)
                        <div class="ac-footer">
                            <a class="btn btn-primary btn-primary_2" style="float: right;"  href="{{route('admin.business.info.verify',$user['id'])}}">VERIFY</a>
