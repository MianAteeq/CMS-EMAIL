...
<div class="row">
    <div class="col-md-3">
        ...
        <p style="color: black;padding-left: 25px;">Status :  @if ($user['application_status']=="IN_REVIEW") Info in Review @else {{$user['application_status']}}  @endif</p>
          <p style="color: black;padding-left: 25px;">Verification status :  @if($user['profile']['main_account_status']==0)  Pending @elseif ($user['profile']['main_account_status']==1) Accepted  @else Decline @endif</p>

          @if($user['profile']['main_account_status']==0)
          <div class="footers" style="border-top: 2px solid black;padding: 10px ">
            <button class="btn btn-primary btn-block" onclick="window.location.href=`{{route('admin.main.account.verify',$user['id'])}}`">ACCEPT</button>
            <button class="btn btn-primary btn-block" onclick="window.location.href=`{{route('admin.main.account.decline',$user['id'])}}`">DECLINE</button>
            ...
            class="collapse show" aria-expanded="false">
                <div class="card-content">
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-sm-5">
                                <h6 class="mb-0">Authorised person</h6>
                            </div>
                            <div class="col-sm-7 text-secondary">
                        ...
                        <div class="row">
                            <div class="col-sm-5">
                                <h6 class="mb-0">Authorisation proof</h6>
                            </div>
                            <div class="col-sm-7 text-secondary">
                                {{$user['profile']['proof_of_main_contact_name']??'N/A'}} <a class="btn btn-primary btn-primary_2" style="border-color: #ff6600 !important; background-color: #ff6600 !important;float: right;" target="_blank" href="{{URL::to($user['profile']['proof_of_main_contact']??'')}}">View</a>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-7 text-secondary">
                             @if($user['profile']['main_account_status']==0)  Pending @elseif ($user['profile']['main_account_status']==1) Accepted  @else Decline @endif
                            </div>
                        </div>
                        @if($user['profile']['main_account_status']==0)
                        <div class="ac-footer">
                            <a class="btn btn-primary btn-primary_2" style="float: right;"  href="{{route('admin.main.account.info.verify',$user['id'])}}">VERIFY</a>
